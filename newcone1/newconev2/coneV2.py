...
while cap.isOpened():
    ...
    for j in range(len(detections)):
        ...
        # masked[detections[j][2][0]:detections[j][2][1],detections[j][2][2]:detections[j][2][3]].shape
        # cv2.imshow("sep",masked)
    
    
        # print(detections[j][2][0:2])
        
        cv2.circle(frame,(detections[j][2][0],detections[j][2][1]), 5, (0,0,255), -1)
        cv2.circle(frame,(detections[j][2][2],detections[j][2][3]), 5, (0,255,255), -1)
        # frame[]
        ############################################
    # for j in range(len(detections)):
            
        z = frame[detections[j][2][0]:detections[j][2][2],detections[j][2][1]:detections[j][2][3]]
        
        if z.shape[0] != 0 and z.shape[1]!=0:
            
            
            # cv2.rectangle(z,(detections[j][2][0:2]),(detections[j][2][2:4]),(255,0,255),1)
            # cv2.imshow("z",z)
            #z = masked#[detections[j][2][0]:detections[j][2][1],detections[j][2][2]:detections[j][2][3]]
